...
function App() {
  ...
  useEffect(() => {
    authService.onAuthStateChanged(async (user) => {  //onAuthStateChanged는 사용자의 로그인 상태의 변화를 관찰하는 관찰자를 추가시킨다는 의미 
  ...
  return <>
    ...
    {init ? <AppRouter refreshUser={refreshUser} isLoggedIn={Boolean(userObj)} userObj={userObj} /> : "initilaizeing.."}
